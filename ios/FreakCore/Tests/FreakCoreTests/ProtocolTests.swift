// ProtocolTests.swift — pure codec unit tests beyond the golden vectors:
// bounds, masking, parse rejection, name decode/validation edges, Preset
// validation order.
...
@Suite("FreakProtocol codec")
struct ProtocolTests {

    // ---------------------------------------------------------- addressing

    @Test func addressing() throws {
        let a0 = try FreakProtocol.addr(0)
        #expect(a0.bank == 0 && a0.pos == 0)
        let a511 = try FreakProtocol.addr(511)
        #expect(a511.bank == 3 && a511.pos == 127)
        let a384 = try FreakProtocol.addr(384)
        #expect(a384.bank == 3 && a384.pos == 0)
        #expect(throws: FreakError.slotOutOfRange(slot: -1)) {
            try FreakProtocol.addr(-1)
        }
        #expect(throws: FreakError.slotOutOfRange(slot: 512)) {
            try FreakProtocol.addr(512)
        }
        for slot in [0, 127, 128, 383, 384, 511] {
            let (bank, pos) = try FreakProtocol.addr(slot)
            #expect(try FreakProtocol.slot(bank: Int(bank), pos: Int(pos)) == slot)
        }
        #expect(throws: FreakError.slotOutOfRange(slot: 512)) {
            try FreakProtocol.slot(bank: 4, pos: 0)
        }
        #expect(throws: FreakError.self) {
            try FreakProtocol.slot(bank: 0, pos: 128)
        }
        #expect(throws: FreakError.self) {
            try FreakProtocol.slot(bank: -1, pos: 0)
        }
        let e = expectFreakError { try FreakProtocol.slot(bank: 5, pos: 0) }
        #expect(e?.isProtocolError == true)
    }

    // -------------------------------------------------------------- frames

    @Test func frameMasksTo7Bits() {
        let raw = FreakProtocol.frame(seq: 0x85, length: 0x83, cmd: 0x99,
                                      data: Data([0x80, 0xFF, 0x7F]))
        let b = [UInt8](raw)
    ...
    @Test func parseRejectsNonMicroFreakTraffic() {
        // too short
        #expect(FreakProtocol.parse(Data([0xF0, 0x00, 0x20, 0x6B, 0x07, 0x01, 0x00, 0xF7])) == nil)
        // not SysEx at all
        #expect(FreakProtocol.parse(Data([0x90, 0x3C, 0x40])) == nil)
        // missing F7 terminator
        var raw = [UInt8](FreakProtocol.goFrame())
        raw[raw.count - 1] = 0x00
        #expect(FreakProtocol.parse(Data(raw)) == nil)
        // wrong manufacturer (Roland, not Arturia)
        #expect(FreakProtocol.parse(Data([0xF0, 0x41, 0x10, 0x42, 0x12, 0x40,
                                          0x00, 0x7F, 0x00, 0x41, 0xF7])) == nil)
        // right length, wrong device-id byte in the 6-byte prefix
        #expect(FreakProtocol.parse(Data([0xF0, 0x00, 0x20, 0x6B, 0x06, 0x01,
                                          0x01, 0x03, 0x19, 0x00, 0x00, 0x00, 0xF7])) == nil)
        // prefix trailing byte must be 0x01 (full 6-byte match)
        #expect(FreakProtocol.parse(Data([0xF0, 0x00, 0x20, 0x6B, 0x07, 0x02,
                                          0x01, 0x03, 0x19, 0x00, 0x00, 0x00, 0xF7])) == nil)
    }

    @Test func parseFields() throws {
        let raw = try FreakProtocol.readNameRequest(seq: 9, slot: 200)
        let f = try #require(FreakProtocol.parse(raw))
        #expect(f.seq == 9 && f.length == 3 && f.cmd == FreakProtocol.cmdOpen)
        #expect(f.data == Data([1, 72, 0]))
        #expect(f.raw == raw)
        #expect(!f.isChunk && !f.isLastChunk && !f.isAck)
    }

    @Test func frameKindPredicates() {
        let more = FreakProtocol.frame(seq: 1, length: 0x20,
                                       cmd: FreakProtocol.cmdChunkMore,
                                       data: Data(count: 32))
        let last = FreakProtocol.frame(seq: 2, length: 0x20,
                                       cmd: FreakProtocol.cmdChunkLast,
                                       data: Data(count: 32))
        let ack = FreakProtocol.frame(seq: 3, length: 0, cmd: FreakProtocol.cmdNext)
        let m = FreakProtocol.parse(more)!
        let l = FreakProtocol.parse(last)!
        let a = FreakProtocol.parse(ack)!
        #expect(m.isChunk && !m.isLastChunk && !m.isAck)
        #expect(l.isChunk && l.isLastChunk && !l.isAck)
        #expect(!a.isChunk && a.isAck)
    }
    ...
    @Test func decodeNameReplyErrors() throws {
        // wrong cmd
        let ack = FreakProtocol.parse(FreakProtocol.frame(
            seq: 0, length: 0, cmd: FreakProtocol.cmdNext))!
        let e1 = expectFreakError { try FreakProtocol.decodeNameReply(ack) }
        #expect(e1?.isProtocolError == true)
        // wrong payload length (short 0x52 open-write form)
        let short = FreakProtocol.parse(try FreakProtocol.openWriteFrame(seq: 1, slot: 3))!
        let e2 = expectFreakError { try FreakProtocol.decodeNameReply(short) }
        #expect(e2?.isProtocolError == true)
        // out-of-range embedded address (bank 5 = slot 640) throws, and the
        // error is a ProtocolError so the session discards it as malformed
        let bogus = FreakProtocol.parse(FreakProtocol.frame(
            seq: 0, length: 0x23, cmd: FreakProtocol.cmdName,
            data: Data([5, 0, 0]) + Data(count: 32)))!
        let e3 = expectFreakError { try FreakProtocol.decodeNameReply(bogus) }
        #expect(e3 == .slotOutOfRange(slot: 640))
        #expect(e3?.isProtocolError == true)
    }
    ...
    @Test func nameDecodeSplitsFiltersStrips() throws {
        ...
        field += [UInt8](repeating: 0x41, count: FreakProtocol.nameLen - field.count)
        let payload = Data([0, 7, 0]) + Data(count: 9) + Data(field)
        let f = FreakProtocol.parse(FreakProtocol.frame(
            seq: 0, length: 0x23, cmd: FreakProtocol.cmdName, data: payload))!
    ...
    @Test func validateNameRules() {
        #expect(throws: Never.self) { try FreakProtocol.validateName("") }
        #expect(throws: Never.self) { try FreakProtocol.validateName("Akiko San") }
        #expect(throws: Never.self) {
            try FreakProtocol.validateName("ABCDEFGHIJKLMNOPQRSTUVW")  // 23 chars
        }
        let tooLong = expectFreakError { try FreakProtocol.validateName("ABCDEFGHIJKLMNOPQRSTUVWX") }
        ...
        for bad in [" Foo", "Foo ", "Fo\to", "Café"] {
            let e = expectFreakError("name \(bad)") { try FreakProtocol.validateName(bad) }
    ...
    @Test func nameWriteFrameErrors() throws {
        ...
        let e1 = expectFreakError {
            try FreakProtocol.nameWriteFrame(seq: 1, slot: 0, name: " x", meta: goodMeta)
        }
        ...
        let e2 = expectFreakError {
            try FreakProtocol.nameWriteFrame(seq: 1, slot: 0, name: "X",
                                             meta: Data(count: 8))
        }
        ...
        // slot out of range
        #expect(throws: FreakError.slotOutOfRange(slot: 512)) {
            try FreakProtocol.nameWriteFrame(seq: 1, slot: 512, name: "X", meta: goodMeta)
        }
    }
    ...
    @Test func chunkFramesStream() throws {
        let blob = blob7(1)
        let frames = try FreakProtocol.chunkFrames(blob: blob)
        #expect(frames.count == FreakProtocol.chunkCount)
        var assembled = Data()
        for (i, raw) in frames.enumerated() {
            let f = FreakProtocol.parse(raw)!
            #expect(Int(f.seq) == (i + 1) % 128)
            #expect(f.length == 0x20 && f.data.count == 32)
            #expect(f.cmd == (i == FreakProtocol.chunkCount - 1
        ...
        #expect(FreakProtocol.parse(frames[126])!.seq == 127)
        #expect(FreakProtocol.parse(frames[127])!.seq == 0)
        #expect(FreakProtocol.parse(frames[128])!.seq == 1)
    }

    @Test func chunkFramesErrors() {
        #expect(throws: FreakError.blobSize(expected: 4672, actual: 4671)) {
            try FreakProtocol.chunkFrames(blob: Data(count: 4671))
        }
        var dirty = [UInt8](Data(count: FreakProtocol.blobSize))
        dirty[100] = 0x80
        let e = expectFreakError { try FreakProtocol.chunkFrames(blob: Data(dirty)) }
    ...
    @Test func assembleBlobEnforcesSize() throws {
        let chunk = FreakProtocol.parse(FreakProtocol.frame(
            seq: 1, length: 0x20, cmd: FreakProtocol.cmdChunkMore,
            data: Data(count: 32)))!
        #expect(throws: FreakError.blobSize(expected: 4672, actual: 64)) {
            try FreakProtocol.assembleBlob([chunk, chunk])
        }
    }

    @Test func digestIsLowercaseSha256Hex() {
        // sha256 of the empty string — a universally known value
        #expect(FreakProtocol.digest(Data())
            == "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855")
    }

    @Test func noChecksumIsLaw() {
        #expect(FreakProtocol.noChecksum)
    }
    ...
    @Test func presetShaAndRename() throws {
        ...
        #expect(preset.sha256 == FreakProtocol.digest(blob))
